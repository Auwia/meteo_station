...
def create_db_partition():
    try:
        ...
        cursor.execute(f"SELECT PARTITION_NAME FROM INFORMATION_SCHEMA.PARTITIONS WHERE TABLE_SCHEMA = 'meteo' AND TABLE_NAME = 'humidities' AND PARTITION_NAME = '{partition_name}'")
        partition_exists = cursor.fetchone()

        if partition_exists:
            print(f"{readable_timestamp} : HUMIDITY: Partition {partition_name} already exists.")
            return

        # Check if pmax exists
        cursor.execute("SELECT PARTITION_NAME FROM INFORMATION_SCHEMA.PARTITIONS WHERE TABLE_SCHEMA = 'meteo' AND TABLE_NAME = 'humidities' AND PARTITION_NAME = 'pmax'")
        ...
        if pmax_exists:
            ...
            alter_query = f"ALTER TABLE humidities ADD PARTITION (PARTITION {partition_name} VALUES LESS THAN ({partition_value}))"

        cursor.execute(alter_query)
        connection.commit()
        print(f"{readable_timestamp} : HUMIDITY: Partition {partition_name} added successfully.")

    except Error as e:
        print(f"{readable_timestamp} : HUMIDITY: Error: {e}")
    finally:
